...
class FoodCollectionService:
    ...
    def serve_by_flat_lookup(db: Session, *, event_id, flat_id, performed_by):
        ...
        if not token:
            ...
            total_entitlement = food_pass.veg_count + food_pass.jain_count + food_pass.kids_count
            token_served_count = (
                db.query(func.count(EventFoodToken.id))
                .filter(
                    EventFoodToken.event_id == event_id,
            ...
            fallback_served_count = (
                ...
                .filter(
                    ...
                    AuditLog.action == "SERVE_FOOD_FLAT_FALLBACK",
                )
                .scalar()
            ) or 0

            remaining = total_entitlement - token_served_count - fallback_served_count
            if remaining <= 0:
                raise Exception("No remaining tokens for this flat")
            ...
            db.add(
                AuditLog(
                    ...
                    entity_type="food_collection",
                    entity_id=flat_id,
                    action="SERVE_FOOD_FLAT_FALLBACK",
                    reason="Served via flat lookup without token",
                    performed_by=performed_by,
                )
            )
            db.commit()
            return SimpleNamespace(token_code="FALLBACK", food_type="fallback", flat_id=flat_id)
    ...
    def dashboard(db: Session, *, event_id, recent_limit: int = 10):
        ...
        flat_ids = {row.flat_id for row in served_tokens}
        flat_rows = db.query(Flat).filter(Flat.id.in_(tuple(flat_ids))).all() if flat_ids else []
        flat_lookup = {row.id: row.flat_number for row in flat_rows}
            ...
            "recent_served": [
                {
                    "token": row.token_code,
                    "flat_id": row.flat_id,
                    "flat_number": flat_lookup.get(row.flat_id),
                    "food_type": row.food_type,
