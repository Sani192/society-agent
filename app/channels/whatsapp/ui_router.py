...
def _send_food_token_picker(
    *,
    ...
    query = db.query(EventFoodToken).filter(EventFoodToken.event_id == event_id)
    if not include_served:
        query = query.filter(EventFoodToken.served_at.is_(None))
    tokens = query.order_by(EventFoodToken.created_at.asc()).limit(8).all()
    if not tokens:
    ...
    rows = [
        {
            ...
            "description": (
                f"{token.food_type.title()} | {'Served' if token.served_at is not None else 'Pending'}"
            ),
        }
        for token in tokens
    ]
