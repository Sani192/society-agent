...
def _format_conversational_options(options: list[dict]) -> str:
    lines = [
        ...
        "Reply with `export <number>` or `choose report <key>`.",
    ]
    for index, option in enumerate(options, start=1):
        lines.append(
            f"{index}. {option['label']} ({option['category']}/{option['report_key']})"
        )
    lines.append("Then send `format <pdf|csv|excel>` and optional `event <event-id>`.")
    lines.append(
        "Advanced syntax still works: report export --category ... --report ... --format ..."
    )
...
def handle_committee_intent(
    *,
    ...
    if intent in {"EXPORT_SELECTION", "CHOOSE_REPORT"}:
        ...
        elif intent == "CHOOSE_REPORT" and len(tokens) >= 3:
            key = tokens[2]
            selected_option = next(
                (option for option in session.options if option["report_key"] == key),
                None,
            )

        if not selected_option:
            return error_response(
                "Invalid report selection. Reply with a valid number from `report options` or `choose report <key>`."
            )

        session.selected_category = selected_option["category"]
        session.selected_report = selected_option["report_key"]
        save_export_session(session_key, session)
        return success_response(
            f"Selected {selected_option['label']}. Now send `format <pdf|csv|excel>`."
        )
    ...
    if intent == "REPORT_EVENT":
        ...
        tokens = (message or "").strip().split(maxsplit=1)
        if len(tokens) < 2 or not tokens[1].strip():
            return error_response("Please provide event id. Example: event evt-2")

        session.event_id = tokens[1].strip()
        save_export_session(session_key, session)
        return success_response(
            f"Event context set to {session.event_id}. Continue with `format <pdf|csv|excel>`."
        )
    ...
    if intent == "REPORT_FORMAT":
        ...
        tokens = (message or "").strip().lower().split()
        if len(tokens) < 2:
            return error_response("Please provide format. Example: format pdf")

        report_format = tokens[1]
        if report_format not in {"pdf", "csv", "excel"}:
            return error_response("Invalid format. Use one of: pdf, csv, excel")

        if not session.selected_category or not session.selected_report:
            session.report_format = report_format
            save_export_session(session_key, session)
            return info_response(
                "Format saved. Now choose report with `export <number>` or `choose report <key>`."
            )
        ...
        try:
            result = WhatsAppReportExportService.export(
                ...
                report=session.selected_report,
