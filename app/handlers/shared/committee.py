...
def handle_committee_intent(
    *,
    ...
    if intent == "GENERATE_FOOD_TOKENS":
        ...
        try:
            AnnouncementManager.queue(
                ...
                event=event,
                message_body=(
                    f"Food tokens are ready for *{event.name}*. "
                    "Send *my tokens* to view and share them."
                ),
                scope="event",
            )
        except Exception:
            logger.exception("Failed to broadcast token generation message")
    ...
    if intent == "OPEN_FOOD_COUNTER":
        ...
        try:
            AnnouncementManager.queue(
                ...
                event=event,
                message_body=(
                    f"Food counter is now open for *{event.name}*. "
                    f"Please keep tokens/QR ready. Closes at: {closes_at_text}."
                ),
                scope="event",
            )
        except Exception:
            logger.exception("Failed to broadcast food counter open message")
    ...
    if intent == "FOOD_DASHBOARD":
        ...
        recent_lines = [
            f"{row['token']} | {row.get('flat_number') or row.get('flat_id')} | {row['food_type']} | {format_datetime(row['served_at'])}"
